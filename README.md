# HM14 — выражения, парсеры, обобщённые вычисления (Java)

Код в `src/` — учебный проект (типичная линия заданий ITMO *Prog-intro / expressions* с тестовым раннером Кorneeva в `base/` и `expression.common`). Ниже описание **по факту кода**: что где лежит, как устроен разбор, что можно вычислить, чем ветки отличаются.
...
Файл не дублирует условия задания курса: только соответствие коду в `src/`. Если правила приоритета или допустимых символов поменяют в конкретной сдаче — смотрите итоговую грамматику в `ExpressionParser` выбранной ветки.
